                    ...
                    <tbody class="font-14">
                        @foreach ($provision as $key => $val)
                            <tr>
                                ...
                                <td><a href="{{ route('dsc.provisional-registration-view', $val->app_id) }}" class="btn-bordered rounded-pill waves-effect waves-light btn btn-primary" title="View semester progress details">View
                                    </a>
                                    @if($val->status == 15)
                                    <a href="{{ route('je.notify-reg-office-order', $val->app_id) }}"
                                        class="btn rounded-pill waves-effect waves-light btn btn-pink" title="View office order">OFFICE ORDER
                                    </a>
                                    @endif
                                </td>
                            </tr>
