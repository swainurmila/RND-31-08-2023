...
@section('content')
@section('heading', 'Supervisor Application')
                    ...
                    <tbody class="font-14">
                        @foreach ($applications as $key => $value)
                            <tr>
                                ...
                                <td>{{ $value->age }}</td>
                                <td><span class="badge bg-warning">{{app\Helpers\Helpers::SupervisorApplicationStatus($value->id)}}</span></td>
                                <td class="table-action">
                                    @if(session('userdata')['role_id'] == '1')
                                    <a href="{{route('admin.sup.preview-application', ['id' => $value->sup_id])}}" class="action-icon"> <i class="mdi mdi-eye"></i></a>
                                    @else
                                    <a href="{{route('control-cell.sup.preview-application', ['id' => $value->sup_id])}}" class="action-icon"> <i class="mdi mdi-eye"></i></a>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
