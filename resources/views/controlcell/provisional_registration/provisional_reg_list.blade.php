                    ...
                    <tbody class="font-14">
                        @foreach ($provision as $key => $val)
                            <tr>
                                ...
                                <td>@if(session('userdata')['role_id'] == '7')
                                    <a href="{{ route('control-cell.provisional-registration-view', $val->app_id) }}"
                                        class="btn-sm rounded-pill waves-effect waves-light btn btn-primary" title="View semester progress details">View
                                    </a>
                                    @if($val->status == 15)
                                    ...
                                    <a href="{{ route('vc.provisional-registration-view', $val->app_id) }}"
                                        class="btn-sm rounded-pill waves-effect waves-light btn btn-primary" title="View semester progress details">View
                                    </a>
                                    @if($val->status == 15)
                                    <a href="{{ route('je.notify-reg-office-order', $val->app_id) }}"
